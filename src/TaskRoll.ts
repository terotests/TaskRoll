...
export default class TaskRoll {
  ...
  cond(
    condition: AnyFunction | TaskRollFn | TaskRoll,
    fn: AnyFunction | TaskRollFn | TaskRoll | any,
    elseFn?: any
  ): TaskRoll {
  ...
  resolve(ctx: TaskRollCtx) {
    ...
    const parallels_exited = (key: string, task?: TaskRoll) => {
      ...
      if (
        task.type == TaskRollType.Parallel &&
    ...
    if (ctx.task) {
      if (ctx.task.type == TaskRollType.Parallel) {
        if (
          parallels_exited("prev", ctx.task) &&
  ...
  step(ctx: TaskRollCtx) {
    ...
    const resolve_task = (nextTask: TaskRoll) => {
      nextTask.state = TaskRollState.Running;
      let anotherTask = nextTask.executeTask(
        ctx
          .setParent(this)
          .setTask(nextTask)
          .setThread(nextTask)
      ...
      while (anotherTask) {
        ...
        anotherTask = anotherTask.executeTask(
          ctx
            .setParent(this)
            .setTask(nextTask)
  ...
  async endGracefully(ctx: TaskRollCtx) {
    ...
    if (this.ctx.parent) {
      ...
      if (this.isolated) {
        ...
        this.ctx.parent.resolve(
          this.ctx.setValue(ctx.value).reduceState(ctx.state)
        );
      }
    }
    this.state = TaskRollState.Resolved;
